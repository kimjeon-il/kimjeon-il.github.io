...
function overviewStructureDescriptionText(partLabel) {
  if (!partLabel) return "";
  for (const group of overviewStructureGroups) {
    for (const structure of group.structures || []) {
      const match = (structure.descriptions || []).find(entry => entry.part === partLabel && entry.text);
...
function modalTagDescription(tag) {
  const typeDescription = overviewTypeDescriptions.find(entry => entry.tag === tag);
  if (typeDescription) return typeDescription.text;
  const spinDescription = overviewSpinDescriptions.find(entry => entry.tag === tag);
  if (spinDescription) return spinDescription.text;
  return overviewStructureDescriptionText(structureTagPartLabels[tag]);
}
...
const modalTagMarkup = tag => {
  const label = tagLabels[tag] || tag;
  const description = modalTagDescription(tag);
  return description
...
function beyModalTags(item) {
  return modalTagGroup(modalTags(item), "bey-modal-tags");
}
...
const overviewTypeDescriptions = [
  { tag: "ATTACK", label: "어택형", text: "공격에 특화되어 스태미나형에 유리하다" },
  { tag: "DEFENSE", label: "디펜스형", text: "방어에 특화되어 어택형에 유리하다" },
  { tag: "STAMINA", label: "스태미나형", text: "지구력이 높아 디펜스형에 유리하다" },
  { tag: "BALANCE", label: "밸런스형", text: "공격·방어·지구력을 고르게 갖추고 있다" }
];
const overviewSpinDescriptions = [
  { tag: "RIGHT SPIN", label: "우회전", text: "시계 방향으로 회전한다" },
  { tag: "LEFT SPIN", label: "좌회전", text: "반시계 방향으로 회전한다" },
  { tag: "DUAL SPIN", label: "양회전", text: "좌우회전 모두 가능하다" }
];
const overviewDescriptionListMarkup = entries => `<ul>${entries.map(entry => `<li><span>${escapeHtml(entry.label)}</span>${escapeHtml(entry.text)}</li>`).join("")}</ul>`;
  ...
  if (activeOverviewGuideTab === "type") return `
    <div class="overview-guide-info">
      <div class="overview-guide-copy">
        <section>
          <h4>타입</h4>
          ${overviewDescriptionListMarkup(overviewTypeDescriptions)}
        </section>
      </div>
    </div>`;
  ...
  return `
    <div class="overview-guide-info">
      <div class="overview-guide-copy">
        <section>
          <h4>회전방향</h4>
          ${overviewDescriptionListMarkup(overviewSpinDescriptions)}
        </section>
      </div>
    </div>`;
};
  ...
  content.innerHTML = `<div class="modal-inner overview-anime-modal">
    <div class="modal-info overview-anime-info">
      ...
      <div class="overview-title-row anime-episode-title-row">
        <h3 class="overview-release-title">${escapeHtml(animeEpisodeTitle(episode, backAnimeRegion))}</h3>
      </div>
    </div>
  </div>`;
  content.querySelector("[data-back-anime-episodes]")?.addEventListener("click", () => {
    openOverviewAnimeDetail({
      animeTab: "episodes",
...
document.querySelector(".overview-panel")?.addEventListener("click", event => {
  const overviewGuideButton = event.target.closest("[data-overview-guide-open]");
  if (overviewGuideButton) {
    event.preventDefault();
    openOverviewGuideDetail();
    return;
  }

  const overviewReleaseButton = event.target.closest("[data-overview-release-open]");
  if (overviewReleaseButton) {
    event.preventDefault();
    openOverviewReleaseDetail();
    return;
  }

  const overviewAnimeButton = event.target.closest("[data-overview-anime-open]");
  if (overviewAnimeButton) {
    event.preventDefault();
    openOverviewAnimeDetail();
    return;
  }
...
function openDetail(id, options = {}) {
  ...
  bindModalTagPopovers(document.querySelector("#modalContent"));
