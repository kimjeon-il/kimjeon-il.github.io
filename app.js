...
const partItems = [
  ...
  { id: "BOTTOM-BALL", series: "metal fight", type: "bottom", name: "B", en: "Ball", sub: "볼", battleType: "defense", tags: [], desc: "접지력이 강한 축의 끝을 가지고 있어서 밸런스와 방어력이 우수하다.", stats: [30, 30, 10], modelKey: "BO_B" },
...
const categoryAnimeTabsMarkup = () => {
  ...
  ${tabs.map(tab => `<button type="button" role="tab" aria-selected="${activeCategoryAnimeTab === tab.id ? "true" : "false"}" class="ui-tab-button ${activeCategoryAnimeTab === tab.id ? "active" : ""}" data-category-anime-tab="${tab.id}">${tab.label}</button>`).join("")}
</div>`;
};
